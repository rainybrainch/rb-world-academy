...
export async function GET() {
  // ===== Production環境: 絶対にフォールバックを返す =====
  if (process.env.NODE_ENV === 'production') {
    try {
      // Production でも実際に読み込んでみる
      if (ACADEMIES_PATH) {
        const entries = readdirSync(ACADEMIES_PATH);
        // ... 実装続く
      }
    } catch (error) {
      // Production なら何が起きてもフォールバック
      console.warn('[Production] Returning fallback academies:', error instanceof Error ? error.message : String(error));
    }

    // フォールバックデータを返す（絶対にエラーにしない）
    return NextResponse.json({
      academies: createFallbackAcademies(),
      totalAcademies: 10,
      totalApps: 0,
      source: 'fallback',
      environment: 'production',
      message: 'Running in production with fallback data',
    });
  }
  ...
  try {
    ...
    console.error('[Development] Error reading Academies:', error);

    // Development環境でもエラーが起きたら詳細を返す
    return NextResponse.json(
      {
        error: 'Failed to read Academies folder',
